...
/**
 * Produces canonical JSON metadata stored beside a plain-text comment.
 * Invalid, stale and duplicate entries are removed.
 */
  ...
  references.forEach((reference) => {
    ...
    if (existingAnnotationId && existingAnnotationId !== reference.annotationId) {
      ...
      Array.from(uniqueReferences.entries()).forEach(([key, value]) => {
        if (value.reference.label === reference.label) uniqueReferences.delete(key)
      })
      return
    }
    ...
    if (!uniqueReferences.has(key)) uniqueReferences.set(key, { reference, index })
  })
