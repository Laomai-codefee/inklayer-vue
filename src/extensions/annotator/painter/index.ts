...
import { SelectionSource } from '@/stores/annotationStore'
import type { useAnnotationStore as UseAnnotationStoreType } from '@/stores/annotationStore'
...
import type { AnnotationPermissionAction, AnnotationPermissions, PdfAnnotatorOptions } from '../types/annotator'
...
export class Painter {
    ...
    private store: ReturnType<typeof UseAnnotationStoreType>
    /**
     * 构造函数，初始化 PDFViewerApplication, EventBus, 和 WebSelection
     * @param params - 包含 store、PDFViewerApplication 和 EventBus 的对象
     */
    constructor({
        primaryColor,
        defaultOptions,
        ...
        this.selector = new Selector({
            ...
            onChanged: async (id, groupString, _rawAnnotationStore, konvaClientRect, transformerRect) => {
                ...
                const currentAnnotation = this.store.getAnnotation(id)
                if (editor) {
                    this.updateStore(id, { konvaString: groupString, konvaClientRect }, false)
                }
                if (currentAnnotation) {
                    this.store.setSelectedAnnotation(currentAnnotation, SelectionSource.SIDEBAR)
                }
    ...
    private setDefaultMode = () => {
        this.store.setCurrentAnnotationType(annotationDefinitions[0])
    }
    ...
    public activate(annotation: IAnnotationType | null, dataTransfer: string | null): void {
        ...
        switch (annotation.type) {
            ...
            // HIGHLIGHT/UNDERLINE/STRIKEOUT 走 default 分支，不提升 Konva z-index
            // 它们通过 WebSelection (DOM 级别文本选择) + SelectionBar 触发 highlightRange 来工作
            default:
                this.setMode('default') // 设置默认模式
                break
        }
    ...
    public getData() {
        return Array.from(this.store.annotations.values())
    }
