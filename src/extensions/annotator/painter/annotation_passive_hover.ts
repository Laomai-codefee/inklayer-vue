...
export class AnnotationPassiveHover {
  ...
  public registerPage(
    ...
    const page = {
      ...
      pointerMove: (event: PointerEvent) => {
        this.handlePointerMove(pageNumber, event)
      },
      pointerLeave: (event: PointerEvent) => {
        this.handlePointerLeave(pageNumber, event)
      },
      pendingPointer: null,
      frameId: null,
    } satisfies PassiveHoverPage

    element.addEventListener('pointermove', page.pointerMove, {
      capture: true,
      passive: true,
    })
    element.addEventListener('pointerleave', page.pointerLeave, {
      capture: true,
      passive: true,
    })
  ...
  public clear(): void {
    ...
    this.pages.forEach(page => this.cancelPending(page))
    if (activeHover) this.onHoverEnd(activeHover.annotationId)
  }

  public destroy(): void {
    Array.from(this.pages.keys()).forEach(pageNumber => {
      this.unregisterPage(pageNumber)
    })
    this.clear()
  }
  ...
  private resolvePointer(pageNumber: number): void {
    ...
    const stagePoint = {
      x: (pointer.clientX - containerRect.left)
        * (page.stage.width() / containerRect.width),
      y: (pointer.clientY - containerRect.top)
        * (page.stage.height() / containerRect.height),
    }
